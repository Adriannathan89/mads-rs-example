...
- **Trait** defines the capability required by the application. For example,
  `UserRepository` describes user persistence without exposing SeaORM queries.
...
- **Repository** contains database-specific operations. `UserRepositoryImpl`
  owns SeaORM queries and receives a `DatabaseConnection` supplied by a MADS
  provider.
...
This project is a Rust HTTP API built with MADS.rs, SeaORM, and PostgreSQL. It
provides a user module with a layered dependency structure:

```text
UserController → Arc<dyn UserService> → UserServiceImpl → Arc<dyn UserRepository> → UserRepositoryImpl → SeaORM DatabaseConnection
```

`UserRepositoryImpl` owns the SeaORM queries. The application provider creates
the `DatabaseConnection` from `database.url`; the module registers the concrete
...
The included SQL migration creates the `users` and `posts` tables. Apply it to
an empty database before starting the API:

```bash
psql "$DATABASE_URL" -f migrations/01789196348572466463_schema_diff/up.sql
```

SeaORM owns application persistence. MADS's optional Diesel database feature
and its `mads db` commands are intentionally not enabled in this project.
